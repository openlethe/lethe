...
// ---------------------------------------------------------------------------
// Shared Lethe HTTP client helpers
// ---------------------------------------------------------------------------
...
const RecordParams = Type.Object({
  content: Type.String({
    description:
      "The decision or reasoning to record. Be specific: include the what, why, and any constraints considered.",
  }),
  sessionKey: Type.Optional(
    Type.String({ description: "Override the current session key." })
  ),
  tags: Type.Optional(
    Type.Array(Type.String(), {
      description:
        "Optional tags to organize this record (e.g. ['architecture', 'decision'])",
    })
  ),
});
...
const FlagParams = Type.Object({
  content: Type.String({
    description:
      "What the agent is uncertain about — a hypothesis, a guess, or a known gap.",
  }),
  confidence: Type.Number({
    description:
      "Confidence score from 0.0 (complete guess) to 1.0 (certain).",
    minimum: 0,
    maximum: 1,
  }),
  sessionKey: Type.Optional(
...
const SearchParams = Type.Object({
  ...
  sessionKey: Type.Optional(
    Type.String({ description: "Override the current session key for session-scoped search." })
  ),
...
export class LetheTools {
  constructor(private cfg: LetheToolsConfig) {}
  ...
  getRecordTool(): AgentTool<typeof RecordParams> {
    return this.makeTool({
      name: "memory.record",
      description:
        "Record a deliberate decision the agent has made, including the reasoning behind it. Use this for architecture choices, trade-off resolutions, and any conclusions reached during the session.",
      params: RecordParams,
      label: "Record Decision",
      execute: async (toolCallId, params) => {
        const { content, sessionKey, tags } = params as Static<typeof RecordParams>;
  ...
  getLogTool(): AgentTool<typeof LogParams> {
    return this.makeTool({
      name: "memory.log",
      description:
        "Log an ambient observation, event, or note. Lower stakes than record — use this to track what's happening without requiring structured reasoning.",
      params: LogParams,
      label: "Log Observation",
      execute: async (toolCallId, params) => {
        const { content, sessionKey, tags } = params as Static<typeof LogParams>;
        const { endpoint, apiKey, agentId, projectId } = this.cfg;
        const sk = sessionKey ?? agentId;
  ...
  getFlagTool(): AgentTool<typeof FlagParams> {
    return this.makeTool({
      name: "memory.flag",
      description:
        "Flag a knowledge gap, uncertainty, or educated guess. The confidence score surfaces this for human review. Use when you know you're working with incomplete information.",
      params: FlagParams,
      label: "Flag Uncertainty",
      execute: async (toolCallId, params) => {
        const { content, confidence, sessionKey } = params as Static<
          typeof FlagParams
        >;
        const { endpoint, apiKey, agentId, projectId } = this.cfg;
        const sk = sessionKey ?? agentId;
  ...
  getTaskTool(): AgentTool<typeof TaskParams> {
    return this.makeTool({
      name: "memory.task",
      description:
        "Track a task through status transitions (todo → in_progress → done | blocked). Each transition is recorded as a separate event with a parent link, building a full audit trail.",
      params: TaskParams,
      label: "Update Task",
      execute: async (toolCallId, params) => {
        const { title, status, sessionKey, parentEventId } = params as Static<
          typeof TaskParams
        >;
        const { endpoint, apiKey, agentId, projectId } = this.cfg;
        const sk = sessionKey ?? agentId;
  ...
  getSearchTool(): AgentTool<typeof SearchParams> {
    return this.makeTool({
      name: "memory_search",
      description:
        "Search Lethe memory for past decisions, observations, flags, and tasks. " +
      ...
      label: "Search Memory",
      execute: async (toolCallId, params) => {
        const { query, limit, sessionKey, eventType } = params as Static<typeof SearchParams>;
  ...
  private makeTool<T extends TSchema>({
    name,
    description,
    params,
    label,
...
function formatEvent(event: any): string {
  ...
  const tags = event.tags
    ? (Array.isArray(event.tags) ? event.tags : event.tags.split(",")).filter((t: string) => t)
    : [];
